...
console.log('Mapped headers:', mapped);
...
console.log('Rows raw len:', rows.length);
...
csvFormRows.forEach((r,i) => {
    console.log(`Row ${i}: tipo=${r.tipo} pv=${r.pv} numero=${r.numero} fecha=${r.fecha_emision} total=${r.total} subtotal=${r.subtotal} iva=${r.iva_total} trib=${r.tributos_total} moneda=${r.moneda}`);
    const errors = [];
    ...
    // Simulate backend numero construction
    let numero = r.numero || '';
    if (r.pv) numero = `${String(r.pv).padStart(4,'0')}-${numero}`;
    console.log(`  Backend numero constructed: ${numero}, tipo: ${r.tipo}`);
});
...
console.log('Would the frontend show preview with', csvFormRows.length, 'rows?');

            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                ...
                <div class="space-y-2 p-3 sm:hidden">
                    <div v-for="c in cuentas" :key="c.id" class="rounded-lg border border-gray-200 bg-white p-3">
                        <div class="text-xs font-semibold text-gray-900">{{ c.razon_social || '-' }}</div>
                        <div class="text-[10px] text-gray-500">CUIT {{ c.cuit || '-' }}</div>
                        <div class="mt-1 text-xs font-medium text-gray-900 font-mono">Saldo {{ formatNum(c.saldo) }}</div>
                        <div class="mt-1"><Link class="text-xs text-indigo-600 hover:text-indigo-800" :href="route('compras.proveedores.ctacte.show', c.id)">Consultar</Link></div>
                    </div>
                </div>

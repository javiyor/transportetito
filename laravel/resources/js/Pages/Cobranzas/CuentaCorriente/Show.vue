        ...
        <div class="max-w-7xl mx-auto py-4 sm:px-6 lg:px-8 space-y-3">
            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                ...
                <div class="space-y-3 p-4 sm:hidden">
                        <div v-for="c in comprobantes" :key="c.id" class="rounded-lg border p-4" :class="c.is_pagada ? 'border-green-200 bg-green-50/40' : 'border-gray-200 bg-white'">
                ...
                <div class="hidden sm:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50"><tr><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Comprobante</th><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fecha</th><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total</th><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pendiente</th><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Estado</th><th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Acciones</th></tr></thead>
                            <tbody class="bg-white divide-y divide-gray-200"><tr v-for="c in comprobantes" :key="c.id" :class="c.is_pagada ? 'bg-green-50/40' : ''"><td class="px-4 py-2 text-sm text-gray-900">{{ tipoLabel(c.tipo) }} {{ comprobanteNumero(c) }}</td><td class="px-4 py-2 text-sm text-gray-700">{{ formatFecha(c.fecha_emision) }}</td><td class="px-4 py-2 text-sm text-gray-700">{{ c.moneda }} {{ formatNum(c.total) }}</td><td class="px-4 py-2 text-sm" :class="c.is_pagada ? 'text-green-700 font-medium' : 'text-amber-700'">{{ c.moneda }} {{ formatNum(c.pendiente ?? c.total) }}</td><td class="px-4 py-2 text-sm"><span v-if="c.is_pagada" class="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800">Pagada</span><span v-else-if="c.is_credit" class="inline-flex items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">Crédito</span><span v-else class="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">Pendiente</span></td><td class="px-4 py-2 text-right text-sm"><Link v-if="!c.is_credit" class="text-indigo-600 hover:text-indigo-800" :href="route('operacion.comprobantes.show', c.id)">Ver</Link><span v-else class="text-xs text-gray-400 italic">Pago a cuenta</span></td></tr></tbody></table>
                </div>
            </div>
